...
> **不需改動實例化邏輯**：`defaultInstantiate`（[src/models/cards.js](../src/models/cards.js)）以 `...card` 整份展開、`evolveCard`（[src/game/rules.js](../src/game/rules.js)）以 `...evoCard` 展開，故場上實例與進化後寶可夢都會自動帶上這兩個欄位。
